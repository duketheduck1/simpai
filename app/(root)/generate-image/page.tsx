...
import { ViewImage } from "@/components/ViewContent";
import { getContent } from "@/database/content";
import { getTexttoImage } from "@/server/actions/openai";
...
const Page = () => {
  ...
  const idClerk = user?.id || "anonymous-id"; // Fallback to a default value if user is not available
  const type = "IMAGE";
  const date = new Date();
  ...
  const sendInput = async () => {
    ...
    setLoading(true);
    const data = await getTexttoImage({input, idClerk, author});
  ...
  return (
    <section className="w-full h-screen p-4">
      <div className="fixed top-0 flex flex-wrap items-center justify-between gap-2 z-10 bg-gray-400 w-full">
        <h2 className="text-2xl font-semibold p-2 ">Text to Image</h2>
      </div>
      <div className="relative flex-1 h-screen overflow-y-auto p-3 space-y-4 rounded-md w-full ">
        {/* display user's prompt */}
        {msg.map((item, index) => (
          <ViewImage key={index} item={item} index={0} />
        ))}
